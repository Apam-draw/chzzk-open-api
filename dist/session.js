...
export function parseSystemMessage(payload) {
...
export function createSession(config, accessToken) {
    return new ChzzkClient(config).request("/open/v1/sessions/auth", z.object({ url: z.url() }), { accessToken });
}
export async function subscribeDonation(config, accessToken, sessionKey) {
    const query = new URLSearchParams({ sessionKey });
    await new ChzzkClient(config).request(`/open/v1/sessions/events/subscribe/donation?${query}`, z.unknown(), { method: "POST", accessToken });
}
export async function subscribeChat(config, accessToken, sessionKey) {
    const query = new URLSearchParams({ sessionKey }).toString();
    await new ChzzkClient(config).request(`/open/v1/sessions/events/subscribe/chat?${query}`, z.unknown(), { method: "POST", accessToken });
}
export class ChzzkSessionManager {
    config;
    logger;
    socket = null;
    constructor(config, logger = silentChzzkLogger) {
        this.config = config;
        this.logger = logger;
    }
    connect(input) {
        ...
        socket.on("SYSTEM", (payload) => {
            ...
            void subscribeDonation(this.config, input.accessToken, sessionKey)
                ...
                .catch((error) => this.logger.error("CHZZK_SESSION_DISCONNECTED", error));
            if (input.onChat) {
                void subscribeChat(this.config, input.accessToken, sessionKey)
                    .then(() => this.logger.info("CHZZK_CHAT_SUBSCRIBED"))
                    .catch((error) => this.logger.error("CHZZK_CHAT_SUBSCRIBE_FAILED", error));
            }
        });
        if (input.onChat) {
            socket.on("CHAT", (payload) => {
                const raw = parseSocketPayload(payload);
                ...
                try {
                    ...
                    void input.onChat?.(event, raw).catch((error) => {
        ...
        socket.on("DONATION", (payload) => {
            ...
            try {
                ...
                void input.onDonation(event, raw).catch((error) => {
